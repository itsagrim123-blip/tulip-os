window.UserManager = class UserManager {
    constructor(notifications) { this.notifications = notifications; this.key = "tulip.users"; this.sessionKey = "tulip.activeUser"; this.users = this.load(); this.active = localStorage.getItem(this.sessionKey) || "admin"; }
    load() {
    ...
    async seedHome(username) { const previous = window.TulipFS.activeUser; window.TulipFS.setActiveUser(username); for (const path of ["/Desktop", "/Documents", "/Downloads", "/Pictures"]) if (!(await window.TulipFS.get(path))) await window.TulipFS.create(path, "folder"); window.TulipFS.setActiveUser(previous); }
